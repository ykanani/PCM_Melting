...
lwidth=ones(1,100);
cTemp=linspecer(10);
colors=[repelem(cTemp(:,1),4) repelem(cTemp(:,2),4)  repelem(cTemp(:,3),4) ];
r1=results{1};
options=r1.options;
...
TaxisLow=min([options.Tlow options.Thigh])-0.1;
TaxisHigh=max([options.Tlow options.Thigh])+0.1;
...
%     t=options.t0:options.dt:options.t1;
    ...
    q.tot=squeeze(sum(sum(abs(results{k}.Q(2:Nx+1,2:Ny+1,:)),1),2));  % Watts
    q.lat=squeeze(sum(sum(abs(results{k}.S(2:Nx+1,2:Ny+1,:)),1),2));  % J/s , Watts
    q.sen=q.tot-q.lat;
     %% averaging range
     Tmin=options.Nperiods -10;
     Tmax=options.Nperiods;
    indices = find(t>=Tmin*options.period & t<Tmax*options.period);
    ...
    1+options.Ste;
    fprintf('Qtot = %2.3f,Qlat = %2.3f,  1+Ste=%2.3f, ratio =%2.3f\n',QavePerVolume,QavePerVolumeLat,1+options.Ste,QavePerVolume/(1+options.Ste));
...
%     q.surf=PCM.D/2*PCM.k*(results{k}.T(end,end)-results{k}.T(end,end-1))/Mesh.Csize(end);
    ...
    yyaxis left
    set(gca,'YColor',[0 0 0])
    plot(results{k}.time/options.period,results{k}.gammaAvg,'MarkerEdgeColor','none','LineStyle',lstyles{k},'LineWidth',lwidth(k),...
        'color',colors(k,:),'DisplayName',['$\gamma$, $\mathcal{T}$=' num2str(options.period) 's']);
    hold on
    xlabel('$t/\mathcal{T}$ ','Interpreter','latex')
    ylabel('\gamma','FontSize',24)
    axis([options.Nperiods-NperiodsAvg options.t1/options.period -0.05 1.05]);
...
%    axis([0 600 Air.Tlow-0.05*(Air.Thigh-Air.Tlow) Air.Thigh+0.05*(Air.Thigh-Air.Tlow)])
    ...
    yyaxis left
    set(gca,'YColor',[0 0 0])
    plot(t/options.period,q.tot,'MarkerEdgeColor','none','LineStyle',lstyles{k},'LineWidth',lwidth(k),...
        'color',colors(k,:),'DisplayName',['$Q_{tot}$, $\mathcal{T}$=' num2str(options.period) 's']);
    hold on
